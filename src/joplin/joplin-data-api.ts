...
import Note, { parseJoplinNote, NoteBook } from "./note";
import { ID } from "./note-link";
...
export default class JoplinDataApi {
    ...
    public async getNoteBook(query: any, id: string) : Promise<NoteBook> {
        const joplinNote = await joplin.data.get(['folders', id], query);
        return this.buildNote(joplinNote);
    }
